...
let view (model: Model) (jobId: int) =
    ...
    let title = job |> Option.map (fun j -> j.ProviderName) |> Option.defaultValue "Chat"
    let subtitle =
        if model.ProviderTyping then "typing…"
    ...
    (Grid(coldefs = [ Star ], rowdefs = [ Auto; Star; Auto ]) {
        ...
        (Grid(coldefs = [ Auto; Star ], rowdefs = [ Auto; Auto ]) {
            ...
            Label(subtitle)
                .font(size = 13.)
                .textColor(if model.ProviderTyping then Theme.brand else Theme.inkMuted)
        ...
        (ScrollView(
            (VStack(spacing = Theme.gapTight) {
                if List.isEmpty jobMessages then
                    ...
                    Label("Send a note about parking, a gate code, or anything they should know before arriving.")
                        .font(size = 15.)
