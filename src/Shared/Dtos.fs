...
type JobDto =
    { Id: int; CustomerId: int; CustomerName: string
      ...
      IsDemoTracked: bool
      /// The provider has accepted this job. Since `Accepted` leaves the state
      /// `Scheduled`, this is the only thing that tells an accepted job apart
      /// from an unclaimed one — the provider list uses it to move accepted
      /// work out of "Available" and to stop a second job being taken.
      IsAccepted: bool
